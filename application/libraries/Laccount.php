...
class Laccount {
	...
	public function sign_up_as_page()
	{
		$CI =& get_instance();
		$data = array();
...
function imageResize($imageResourceId,$width,$height) {
	...
	public function confirm_email_admin($hashcode)
	{
		$CI =& get_instance();
        $CI->load->model('Users');
        $data = array();
        #it is temprory After Server Remove this
		$result = $CI->Users->verifyEmail($hashcode);
		...
		if($result) {
			...
			$message = 'Dear User,<br><br> Your Account is activated now.<br><br>Thanks';
                $body = $CI->load->view('email/emailverify_email_template_frelincer_notification.php',$data,TRUE);
		        $CI->load->library('email');
		        $CI->email->set_newline("\r\n");
		        $CI->email->from("admin@hirenwork.com");
		        $CI->email->to("admin@hirenwork.com");
		        $CI->email->subject('Account activated');
		        $CI->email->message($body);
		        $CI->email->send(); 
	...
	public function confirm_email_admin_singhup($hashcode)
	{
		$CI =& get_instance();
        $CI->load->model('Users');
        $data = array();
		...
		if($result) {
			...
			$message = 'Dear User,<br><br> Your Account is activated now Hello.<br><br>Thanks';
                $body = $CI->load->view('email/emailverify_email_template_frelincer_notification.php',$data,TRUE);
		        $CI->load->library('email');
		        $CI->email->set_newline("\r\n");
		        $CI->email->from("admin@hirenwork.com");
